<cfcomponent extends="org.corfield.framework" output="false">
	...
	<cffunction name="setupApplication" returntype="any" access="public" output="false" hint="">
		...
		<cfset var services = structNew()>
		<cfset var gateways = structNew()>
		...
		<cfelse>
	 	...
	 	<!--- Initialize gateways --->
		<cfinclude template="/model/gateways/act_initGateways.cfm">
		
		<!--- Initialize services --->
		<cfinclude template="/model/services/act_initServices.cfm">
		
		<!--- Cache services and gateways in application scope --->
		<cfset application.gateways = gateways>
		<cfset application.services = services>
	</cffunction>
	...
	<cffunction name="setupRequest" output="false">
		...
		<cfif NOT session.mobileoff>
			...
			<cfif isMobile>
				<cfif NOT left(cgi.http_host, 1) EQ "m">
				    <cflocation url="http://m.ogov.net/index.cfm?#cgi.query_string#" addToken="false">
				</cfif>
			</cfif>
		</cfif>
		...
		<!--- Init udfs --->
		<!--- <cfinclude template="/model/udfs/act_initUdfs.cfm"> --->
	</cffunction>
